...
states : Signal State
states =
  foldp upstate initState signalPoint
...
elements: Signal (List Element)
elements =
  map2 (\e1 e2 -> [ e1, e2 ]) (myPi states) (map ((\f -> background ++ f) >> collage 512 512) (view states))


main : Signal Element
main =
  map (flow down) elements
...
upstate pt ( ( hits, ps ), ( misses, qs ) ) =
  if (hit pt) then
    ( ( hits + 1, pt :: ps ), ( misses, qs ) )
  else
    ( ( hits, ps ), ( misses + 1, pt :: qs ) )
...
pointsToCircles c ps =
  List.map (pointsToCircle c) ps
...
timeSeed : Signal Int
timeSeed =
  map (inMilliseconds >> round) (every millisecond)
